...
def getWordScore(word, n):
    ...
    score = sum([SCRABBLE_LETTER_VALUES[letter] for letter in word])*word_length
...
#
def isValidWord_REFERENCE(word, hand, wordList):
...
def isValidWord(word, hand, wordList):
    ...
    if word is None:
        ...
        return False
    elif set(list(word)) > set(local_hand.keys()):
        return False   
    
    for letter in word:
        if letter in local_hand.keys():
            local_hand[letter] -= 1
            zero_match = False
        
    if not zero_match and all(v >= 0 for v in local_hand.values()):
        return True
    else:
        return False
#
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        score = 0    
        print("Current Hand:", end="\t"); displayHand(local_hand)
        ...
        if word == '.':
            break
        elif not isValidWord(word, local_hand, wordList):
            print("Invalid word, please try again.")
        else:
            score = getWordScore(word, n)
            total += score
            local_hand = updateHand(local_hand, word)
            print("\"{0}\" earned {1} points. Total: {2} points".format(word,score,total))
            
    if word == '.':
        print("Goodbye! Total score: {0} points.".format(total))
    else:
        print("Run out of letters. Total score: {0} points.".format(total))


def playHand_REFERENCE(hand, wordList, n):
    ...
    while user_input != 'e':
        ...
        if user_input == 'r':
            if not last_hand:
                ...
                playHand(last_hand, wordList, n) # replay with same hand
        elif user_input == 'n':
            last_hand = dealHand(n)
            playHand(last_hand, wordList, n) # play with new hand
        elif user_input not in ['n', 'e', 'r']:
